...
export default function CreateAnswer_Page({}: Props) {
    ...
    const handleCreate = async () => {
        ...
        try {
            const answerCreated = await CreateAnswer(answerData);
            if (!answerCreated.ok) {
                Swal.fire({
                    icon: "error",
                    title:
                        answerCreated.error || "เกิดข้อผิดพลาดในการสร้างเฉลย",
                });
                return;
            }
            ...
            Swal.fire({
                icon: "success",
                title: "สร้างชุดเฉลยสำเร็จ",
                showConfirmButton: false,
                timer: 1500,
            }).then(() => {
                router.push(`/dashboard/answer/${answerCreated.data?.id}`);
            });
        } catch (error) {
            console.error(error);
        }
